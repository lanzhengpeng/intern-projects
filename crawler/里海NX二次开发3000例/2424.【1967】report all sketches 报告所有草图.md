### 【1967】report all sketches 报告所有草图
...
> 这是NX的二次开发代码，主要包括以下功能：
>
> 1. 定义了一个宏UF_CALL，用于调用NX API，并在调用失败时输出错误信息。
> 2. 定义了一个宏RX，用于输出对象的详细信息。
> 3. 实现了一个函数ask_next_sketch，用于遍历指定部件的所有草图。
> 4. 实现了一个函数ask_part_shortname，用于获取部件的简称。
> 5. 实现了一个函数ask_object_type_descriptor，用于获取对象类型和子类型的描述。
> 6. 实现了一个函数report_feature_name_type_and_sysname，用于获取特征的名称、类型和系统名称。
> 7. 实现了一个函数report_object_info，用于输出对象的详细信息。
> 8. 实现了一个函数do_it，用于遍历当前显示部件的所有草图并输出其信息。
> 9. 实现了ufusr函数，作为二次开发的入口点，用于初始化NX系统、执行do_it函数，然后终止NX系统。
> 10. 实现了ufusr_ask_unload函数，返回卸载二次开发代码的方式。
>
> 总体来说，这段代码的主要作用是遍历当前显示部件的所有草图，并输出每个草图的相关信息。
>
