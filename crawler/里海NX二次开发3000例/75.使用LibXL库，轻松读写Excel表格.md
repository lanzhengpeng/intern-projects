...
{	
    ...
    if(book)
    {
        ...
        if(sheet)
        {
            sheet->setCol(0, 0, 20); //设置第1列到第1列的宽度为20
            sheet->writeNum(3, 0, 25.718, format[0]); //写入数据并设置格式  第4行第1列
            ...
            sheet->writeStr(2, 0, L"王牌飞行员_里海");//写入字符串 第3行第1列
            sheet->writeNum(2, 1, 1000);//写入数据 第3行第2列
        }
