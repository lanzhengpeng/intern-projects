### 【1922】report all geometric members of selected reference sets 报告选定引用集的所有几何成员
...
```cpp
    ...
    {  
        ...
        {  
            ...
            {  
                ...
                default:                    /* Picked one 里海译:翻译为：挑了一个。 */  
                    return a + resp - 5;  
                    break;  
            }  
        } while (TRUE);  
    ...
    /*  
        ...
        reference set.  This reports every geometric object that will be brought  
        into a higher level assembly when the reference set is used.  
    */  
    static void report_all_reference_set_member_geometry(tag_t temp_part, tag_t refset)  
    {  
        int  
            cnt = 0,  
            subtype,  
        ...
        while (!UF_CALL(UF_LAYER_cycle_by_layer(1, &object)) &&  
            (object != NULL_TAG))  
        {  
    ...
    {  
        tag_t  
            temp_part = NULL_TAG,  
            refset;  
        while ((refset = choose_a_reference_set("Report ALL member geometry"))  
            != NULL_TAG)  
        {  
            if (temp_part == NULL_TAG)  
                temp_part = create_new_unique_part_quietly();  
            report_all_reference_set_member_geometry(temp_part, refset);  
        }  
        if (temp_part != NULL_TAG)  
...
> 这段NX二次开发代码的主要功能是列出参考集(refset)中包含的所有几何对象，包括参考集成员中的所有组件、面和边。
>
> 主要思路是：
>
> 1. 获取当前工作部件，并创建一个临时部件用于显示参考集成员。
> 2. 遍历当前工作部件中的所有参考集，让用户选择一个参考集，然后列出该参考集中包含的所有几何对象。
> 3. 利用UF_ASSEM_add_part_to_assembly函数将参考集添加到临时部件中，然后遍历临时部件中的所有几何对象，打印出它们的类型。
> 4. 列出完一个参考集后，继续让用户选择下一个参考集，直到用户选择退出。
> 5. 最后关闭临时部件。
>
> 关键函数包括：
>
> 这段代码实现了列出参考集中所有几何对象的功能，对于理解和检查参考集内容非常有用。
>
