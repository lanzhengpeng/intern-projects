### 【1937】report all objects erased in selected view 报告选定视图中删除的所有对象
...
```cpp
    ...
    {  
        ...
        char  
            msg[MAX_LINE_SIZE+1];  
        while ((view = select_a_view("Report all erased objects")) != NULL_TAG)  
        {  
            RX(view);  
            cnt = 0;  
            while (!UF_CALL(UF_VIEW_cycle_objects(view, UF_VIEW_ERASED_OBJECTS,  
                &object)) && (object != NULL_TAG))  
            {  
                RX(object);  
                cnt++;  
            }  
            sprintf(msg, "%d objects erased from view", cnt);  
            uc1601(msg, TRUE);  
        }  
    }  
    /* qq3123197280 */  
...
>
> 1. 错误报告和日志输出：定义了ECHO宏用于输出日志信息，并提供了错误报告函数report_error，用于输出错误代码和调用信息。
> 2. 查询对象信息：提供了查询对象名称、类型、所属部件等信息的函数，如ask_object_type_descriptor、ask_part_shortname等。
> 3. 查询特征信息：提供了查询特征名称、系统名称、类型等信息的函数report_feature_name_type_and_sysname。
> 4. 选择视图：实现了选择视图的函数select_a_view，用于选择当前视图。
> 5. 遍历视图中的删除对象：实现了do_it函数，用于遍历指定视图中的删除对象，并调用report_object_info输出对象信息。
> 6. 初始化和清理：提供了初始化函数ufusr和清理函数ufusr_ask_unload。
> 7. API调用封装：通过宏UF_CALL封装了NX API调用，简化了错误处理。
>
> 总体来说，这段代码实现了NX二次开发中的错误处理、日志输出、对象查询和视图遍历等功能，为NX二次开发提供了基础支持。
>
