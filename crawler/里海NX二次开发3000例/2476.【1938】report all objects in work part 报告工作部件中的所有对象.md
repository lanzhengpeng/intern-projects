### 【1938】report all objects in work part 报告工作部件中的所有对象
...
```cpp
    ...
    {  
        ...
        do  
        {  
            object = UF_OBJ_cycle_all(part, object);  
            RX(object);  
...
> 这段代码是一个NX二次开发示例，主要功能是遍历当前工作部件中的所有对象，并打印出它们的详细信息。
>
> 主要内容包括：
>
> 1. 定义了错误报告函数ECHO()，用于输出错误信息。
> 2. 定义了宏UF_CALL，用于调用NX API函数，并在调用失败时报告错误。
> 3. 定义了函数ask_part_shortname()，用于获取部件的简称。
> 4. 定义了函数ask_object_type_descriptor()，用于获取对象类型描述符。
> 5. 定义了函数report_feature_name_type_and_sysname()，用于报告特征的名字、类型和系统名。
> 6. 定义了函数report_object_info()，用于报告对象的详细信息，包括类型、子类型、名称、拥有部件、可见性、显示属性等。
> 7. 定义了函数do_it()，用于遍历部件中的所有对象，并打印信息。
> 8. 定义了ufusr()函数，这是NX二次开发的入口函数。在这里调用do_it()执行主体功能。
> 9. 定义了ufusr_ask_unload()函数，用于卸载NX Open应用。
>
> 总体来说，这段代码实现了遍历并打印NX部件中所有对象信息的功能，包括名称、类型、拥有部件等。通过这个示例，我们可以学习如何使用NX Open API遍历对象、获取对象属性等。
>
