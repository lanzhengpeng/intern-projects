### UGNX二次开发 获取工作部件下所有对象的类型和子类型 UF_OBJ_ask_type_and_subtype
...
extern DllExport void ufusr(char* param, int* returnCode, int rlen)
{
	...
	while ((tagObj = UF_OBJ_cycle_all(tagWorkPart, tagObj)) != NULL_TAG)
	{
		vecAllObj.push_back(tagObj);
	}
	//获取所有对象的类型
	for (auto it : vecAllObj)
	{
		int iType = 0;
		int iSubType = 0;
		UF_OBJ_ask_type_and_subtype(it, &iType, &iSubType);
		print("iType = %d, iSubType = %d\n", iType, iSubType);
	}
	UF_terminate();
}
...
#define   UF_mechatronics_dynamic_object_table_subtype      30 /* DynamicObjectTable object is used for dynamic object instantiation .*/
