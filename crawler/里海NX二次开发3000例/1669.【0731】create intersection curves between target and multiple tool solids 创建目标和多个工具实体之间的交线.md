### 【0731】create intersection curves between target and multiple tool solids 创建目标和多个工具实体之间的交线

#### 代码

```cpp
/*HEAD CREATE_INTERSECTION_CURVES_BETWEEN_TARGET_AND_MULTIPLE_TOOL_SOLIDS CCC UFUN */
#include <stdio.h>
#include <string.h>
#include <uf.h>
#include <uf_ui.h>
...
static int report_error( char *file, int line, char *call, int irc)
{
    ...
    /*  NOTE:  UF_print_syslog is new in V18 译:根据文档中的描述，UF_print_syslog是一个在V18版本中新增的函数。它的作用是用于打印系统日志信息。 */
...
> 这段NX二次开发代码实现了在目标体和多个工具体之间创建交线曲线的功能。主要步骤包括：
>
> 1. 使用对话框选择目标体和工具体。
> 2. 创建目标体和工具体的副本。
> 3. 检查目标体和工具体之间是否有干涉，如果有则创建交线曲线。
> 4. 提取交线曲线上的所有特征边。
> 5. 删除临时创建的副本。
>
> 代码中使用了NX的UI、模型、装配、对象、波前、SO等模块的API函数，实现了交互选择、模型操作、交线计算等功能。同时，通过错误处理函数，代码能够打印出详细的错误信息，方便调试。总体来说，这段代码实现了在NX中自动创建目标体和工具体之间的交线曲线，提高了效率。
>
