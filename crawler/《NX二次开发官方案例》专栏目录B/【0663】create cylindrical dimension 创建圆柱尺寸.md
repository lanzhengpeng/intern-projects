### 【0663】create cylindrical dimension 创建圆柱尺寸

#### 代码

```cpp
    /*HEAD CREATE_CYLINDRICAL_DIMENSION CCC UFUN */  
    #include <stdio.h>  
    #include <string.h>  
    #include <math.h>  
    #include <uf.h>  
    ...
    {  
        ...
        /*  NOTE:  UF_print_syslog is new in V18 译:根据文档中的提示，UF_print_syslog是V18版本中的新功能，只要求提供翻译，不要添加其他内容。 */  
    ...
    {  
        ...
        mpi[0] = 1; /* Automatic Text/Arrow placement 译:自动文本/箭头放置 */  
        mpi[7] = 1; /* Only automatic text 译:翻译如下：

只有自动文本 */  
        mpi[30] = 5; /* Mid-center 译:Mid-center 的中文意思是“中间”。 */  
        ...
        {  
            UF_CALL(UF_DRF_create_cylindrical_dim(&object[0], &object[1], &text,  
                origin, &dim));  
        }  
    }  
    /* 里海 */  
...
> 这是一段用于在NX中创建圆柱尺寸的二次开发代码。
>
> 主要功能包括：
>
> 1. 提供了错误报告函数，用于在调用NX API失败时打印错误信息。
> 2. 提供了坐标转换函数，用于将绝对坐标系坐标转换为当前视图坐标系坐标，以及反之。
> 3. 提供了询问对象上点的函数，用于获取用户在对象上选择的位置点。
> 4. 提供了获取曲线端点的函数，用于获取曲线的两个端点坐标。
> 5. 提供了确定曲线端点关联性的函数，用于确定用户选择的点更接近曲线的哪个端点。
> 6. 提供了选择标注曲线端点的函数，用于让用户选择标注曲线的端点，并确定关联性。
> 7. 提供了选择位置的函数，用于让用户选择尺寸的放置位置。
> 8. 提供了创建圆柱尺寸的函数，用于根据用户选择的两个曲线端点和位置创建圆柱尺寸。
> 9. 提供了主函数，用于初始化NX环境，调用创建尺寸的函数，并清理环境。
> 10. 提供了卸载函数，用于在卸载时立即清理资源。
>
> 该代码实现了在NX中创建圆柱尺寸的完整流程，包括用户交互和尺寸创建。
>
