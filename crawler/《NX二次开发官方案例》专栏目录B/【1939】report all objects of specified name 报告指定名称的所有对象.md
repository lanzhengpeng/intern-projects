### 【1939】report all objects of specified name 报告指定名称的所有对象
...
```cpp
    ...
    {  
        tag_t  
            object = NULL_TAG;  
        char  
            the_name[MAX_ENTITY_NAME_SIZE+1] = { "" };  
        while (prompt_for_text("Report objects of name?", the_name))  
            while (!UF_CALL(UF_OBJ_cycle_by_name(the_name, &object)) &&  
                (object != NULL_TAG)) RX(object);  
    }  
    /* qq3123197280 */  
...
> 这段代码是一个NX Open C++ API的二次开发示例，其主要功能是遍历模型中所有名称与用户输入匹配的对象，并报告这些对象的信息。
>
> 主要函数和功能包括：
>
> 1. ECHO宏：用于打印信息到日志窗口和系统日志。
> 2. report_error函数：用于报告函数调用错误。
> 3. RX宏和WRITEnT宏：用于报告对象信息。
> 4. ask_part_shortname函数：用于获取部件的简称。
> 5. ask_object_type_descriptor函数：用于获取对象类型描述符。
> 6. WRITE_S宏：用于打印字符串信息。
> 7. report_feature_name_type_and_sysname函数：用于报告特征的名字、类型和系统名。
> 8. report_object_info函数：用于报告对象的详细信息，包括状态、类型、子类型、名称、拥有部件、显示属性等。
> 9. prompt_for_text函数：用于提示用户输入文本。
> 10. do_it函数：主函数，用于遍历所有名称匹配的对象并报告信息。
> 11. ufusr函数：NX Open C++ API的入口函数。
> 12. ufusr_ask_unload函数：用于卸载用户自定义函数库。
>
> 总的来说，这段代码利用NX Open C++ API遍历模型对象，获取对象信息，并打印输出，提供了一个典型的二次开发示例。
>
