### 【1030】delete objects with specified name 删除指定名称的对象
...
```cpp
    ...
    {  
        ...
        strcpy(descriptor, "Unknown type");  /* In case its not found 里海译:如果找不到的话，就回答“如果找不到”即可。 */  
        while (!feof(in_file))  
        {  
    ...
    {  
        ...
        uf_list_p_t  
            to_delete;  
        while (prompt_for_text("Enter name of object to delete", name))  
        {  
            UF_CALL(UF_MODL_create_list(&to_delete));  
            while ((object = ask_next_of_name(name, object)) != NULL_TAG)  
            {  
                RX(object);  
                sprintf(prompt, "Delete EID %d?", object);  
                ask_yes_or_no(prompt, &delete);  
                if (delete) UF_CALL(UF_MODL_put_list_item(to_delete, object));  
            }  
            if ((n = make_an_array(&to_delete, &junk)) > 0)  
            {  
                UF_CALL(UF_OBJ_delete_array_of_objects(n, junk, &statuses));  
                UF_free(junk);  
                cnt = 0;  
                for (ii = 0; ii < n; ii++)  
                    if (statuses[ii] == 0) cnt++;  
                UF_free(statuses);  
                sprintf(prompt, "Deleted %d objects called %s", cnt, name);  
                uc1601(prompt, TRUE);  
            }  
        }  
    }  
...
> 这段代码是一个用于在NX中删除对象的二次开发代码，其功能主要包括：
>
> 1. 初始化和终止NX环境。
> 2. 循环提示用户输入要删除的对象名称，并显示找到的对象。
> 3. 询问用户是否删除每个对象，若删除则将其添加到列表中。
> 4. 删除列表中的所有对象，并显示删除结果。
> 5. 代码使用了大量的NX API函数，例如UF_OBJ_cycle_by_name、UF_MODL_create_list、UF_MODL_put_list_item、UF_OBJ_delete_array_of_objects等，以实现对象查找、列表创建、对象删除等功能。
> 6. 代码使用了UF_CALL宏来调用NX API函数，并在出错时报告错误。
> 7. 定义了ECHO宏用于在列表窗口打印信息。
> 8. 定义了RX宏用于打印对象信息。
> 9. 定义了WRITEnT宏用于打印数组信息。
> 10. 包含了内存分配、数组创建等辅助函数。
> 11. 定义了do_it函数作为主逻辑，用于循环提示用户并删除对象。
> 12. 定义了ufusr函数作为NX二次开发入口。
> 13. 定义了ufusr_ask_unload函数，用于卸载二次开发程序。
>
> 总的来说，这段代码实现了在NX中根据名称查找并删除对象的功能，其代码结构清晰，注释详细，是一个典型的NX二次开发示例。
>
