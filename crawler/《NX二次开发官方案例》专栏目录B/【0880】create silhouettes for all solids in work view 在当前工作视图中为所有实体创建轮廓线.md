### 【0880】create silhouettes for all solids in work view 在当前工作视图中为所有实体创建轮廓线
...
```cpp
    ...
    {  
        ...
        strcpy(descriptor, "Unknown type");  /* In case its not found 译:如果找不到，就回答"找不到"。不要说其他废话。 */  
        while (!feof(in_file))  
        {  
    ...
    {  
        ...
        tag_t  
            ...
            *curves,  
            work_view;  
        char  
            label[MAX_ENTITY_NAME_SIZE+1];  
        UF_CALL(UF_VIEW_ask_work_view(&work_view));  
        while ((body = ask_next_visible_solid(body)) != NULL_TAG)  
        {  
            RX(body);  
            UF_CALL(UF_CURVE_create_silhouette(body, work_view, &count, &curves));  
            RXn(curves, count);  
            if (count > 0) UF_free(curves);  
        }  
    }  
    /* 里海 */  
...
> 这段代码是用于NX的二次开发代码，主要功能是获取NX装配体中所有可见实体的轮廓线。下面是对代码的简要介绍：
>
> 1. 引入头文件：代码引入了NX Open API的头文件，如uf.h、uf_ui.h等，以调用NX提供的函数接口。
> 2. 宏定义：定义了一些宏，如ECHO用于输出信息，UF_CALL用于调用NX函数并处理错误，RX用于打印对象信息等。
> 3. 错误处理函数：report_error函数用于在调用NX函数出错时输出错误信息。
> 4. 获取实体类型描述：ask_object_type_descriptor函数用于获取实体类型和子类型的描述字符串。
> 5. 打印对象信息：report_object_info函数用于打印对象的各种信息，如名称、类型、拥有者等。
> 6. 获取轮廓线：ask_next_visible_solid函数用于遍历装配体中的所有实体，create_silhouette函数用于获取实体的轮廓线。
> 7. 主函数：do_it函数是主函数，用于获取装配体中所有可见实体的轮廓线并打印。
> 8. ufusr函数：ufusr是NX二次开发的入口函数，用于调用主函数。
> 9. 卸载函数：ufusr_ask_unload用于处理二次开发的卸载。
>
> 总的来说，这段代码通过遍历装配体中的所有实体，获取每个实体的轮廓线，并打印实体信息。它展示了NX二次开发的基本流程和常用函数。
>
