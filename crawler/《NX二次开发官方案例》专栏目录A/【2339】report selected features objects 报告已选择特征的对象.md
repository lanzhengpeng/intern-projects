### 【2339】report selected features objects 报告已选择特征的对象
...
```cpp
    ...
    {  
        ...
        switch (type)  
        {  
            case UF_circle_type: /* non-std format in the header file 里海译:非标准格式头文件中的内容 */  
...
> 这段代码是一个NX Open C++ API开发的UF程序，用于在NX中选择特征，并报告与每个特征相关的对象信息。代码的主要功能包括：
>
> 1. 定义了ECHO宏，用于打印信息到日志窗口和系统日志。
> 2. 定义了UF_CALL宏，用于调用NX API函数，并报告错误。
> 3. select_features函数用于选择特征。
> 4. report_error函数用于报告API调用错误。
> 5. ask_part_shortname函数用于获取部件的简称。
> 6. ask_object_type_descriptor函数用于获取对象类型描述。
> 7. write_string_to_listing_window函数用于将字符串打印到日志窗口。
> 8. report_feature_name_type_and_sysname函数用于报告特征的名字、类型和系统名。
> 9. report_object_info函数用于报告对象的信息，包括类型、子类型、名称、拥有部件、显示属性等。
> 10. report_feature_objects函数用于报告特征关联的对象。
> 11. do_it函数用于执行主功能：选择特征，并报告特征对象信息。
> 12. ufusr函数是程序的入口点。
> 13. ufusr_ask_unload函数用于卸载UF程序。
>
> 总的来说，这段代码利用NX的UF API，实现了选择特征并获取特征对象信息的功能。代码结构清晰，功能明确，是一个典型的NX二次开发示例。
>
