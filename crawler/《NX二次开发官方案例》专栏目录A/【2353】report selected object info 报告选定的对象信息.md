### 【2353】report selected object info 报告选定的对象信息
...
> 这段代码是一个NX的二次开发代码，主要功能是选择并报告NX中的对象信息。以下是代码的主要功能：
>
> 1. 定义了一个错误报告函数report_error，用于在函数调用失败时打印错误信息。
> 2. 定义了一个函数mask_add_faces_and_edges，用于设置选择掩码，使得在选择对话框中可以选择面和边。
> 3. 定义了一个函数select_anything，用于打开选择对话框，让用户选择对象，并返回所选对象的tag。
> 4. 定义了一个函数ask_part_shortname，用于获取部件的简称。
> 5. 定义了一个函数ask_object_type_descriptor，用于获取对象类型和子类型的描述字符串。
> 6. 定义了一个函数write_string_to_listing_window，用于在列表窗口打印字符串。
> 7. 定义了一个函数report_feature_name_type_and_sysname，用于报告特征的名称、类型和系统名称。
> 8. 定义了一个函数report_object_info，用于报告对象的各种信息，包括类型、子类型、名称、拥有部件、显示属性等。
> 9. 定义了一个函数do_it，用于循环选择对象，并报告所选对象的信息。
> 10. 定义了ufusr函数，用于初始化NX，调用do_it函数，然后终止NX。
> 11. 定义了ufusr_ask_unload函数，用于卸载用户函数。
>
> 这段代码的主要功能是让用户选择NX中的对象，然后报告所选对象的各种信息。
>
