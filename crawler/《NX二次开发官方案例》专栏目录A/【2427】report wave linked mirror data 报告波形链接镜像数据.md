### 【2427】report wave linked mirror data 报告波形链接镜像数据
...
```cpp
    ...
    static void report_source( tag_t feat )  
    {  
        tag_t  
            source_body = NULL_TAG,  
            body_xform = NULL_TAG,  
            datum_plane = NULL_TAG,  
            datum_xform = NULL_TAG;  
        if (UF_CALL(UF_WAVE_ask_link_mirror_data(feat, TRUE,  
                &source_body, &body_xform, &datum_plane, &datum_xform))) return;  
        RX( source_body);  
        RX( body_xform);  
        RX( datum_plane);  
        RX( datum_xform);  
    ...
    {  
        tag_t  
            feat = NULL_TAG,  
            part = UF_PART_ask_display_part();  
        while ((feat = ask_next_feature_of_type(part, "LINKED_MIRROR", feat))  
            != NULL_TAG) report_source( feat );  
    }  
    /* qq3123197280 */  
...
> 3. ask_next_feature_of_type函数用于遍历当前部件中的所有特征，并返回指定类型(参数type)的下一个特征。
> 4. ask_part_shortname函数用于获取部件的简称。
> 5. ask_object_type_descriptor函数用于获取NX对象类型和子类型的描述信息。
> 6. report_feature_name_type_and_sysname函数用于输出特征的名字、类型和系统名。
> 7. report_object_info函数用于输出NX对象的信息，包括状态、类型、名称、拥有者等。
> 8. report_source函数用于输出链接镜像特征的相关源对象信息。
> 9. do_it函数用于遍历当前部件中的所有链接镜像特征，并输出它们的源信息。
> 10. ufusr函数是二次开发程序的入口，初始化NX环境，调用do_it函数，然后终止NX环境。
> 11. ufusr_ask_unload函数用于卸载二次开发程序。
>
> 总体来说，这段代码通过NX API遍历部件中的链接镜像特征，并输出这些特征的源信息，以便进行特征分析。
>
