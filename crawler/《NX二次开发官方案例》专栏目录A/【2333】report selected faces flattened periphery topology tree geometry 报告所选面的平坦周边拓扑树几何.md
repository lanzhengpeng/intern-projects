### 【2333】report selected faces flattened periphery topology tree geometry 报告所选面的平坦周边拓扑树几何
...
```cpp
    ...
    {  
        ...
        // Begin work around PR 6547334  
                if ((geometry->geom.spline.start_param != 0.0)  
                    || (geometry->geom.spline.end_param != 1.0))  
                {  
                    ECHO("\n\nConverting natural to normalized data!!\n\n");  
                    UF_CALL(UF_UNDO_set_mark(UF_UNDO_invisible, NULL, &mark_id));  
                    UF_CALL(UF_CURVE_create_spline(  
                        (UF_CURVE_spline_p_t)&geometry->geom.spline,  
                        &curve, &n_states, &states));  
                    if (n_states > 0) UF_free(states);  
                    UF_free(geometry->geom.spline.knots);  
                    UF_free(geometry->geom.spline.poles);  
                    UF_CALL(UF_CURVE_ask_spline_data(curve,  
                        (UF_CURVE_spline_p_t)&geometry->geom.spline));  
                    UF_CALL(UF_UNDO_undo_to_mark(mark_id, NULL));  
                    UF_CALL(UF_UNDO_delete_mark(mark_id, NULL));  
                }  
        // End work around PR 6547334  
    ...
    {  
        ...
        UF_BREP_state_p_t  
            states;  
        static UF_BREP_options_t options;  
        int op_ops[1];  
        double op_data[1];  
        double tol =  0.00254;  
        options.count = 1;  
        options.options = op_ops;  
        options.opt_data = op_data;  
        options.options[0] = UF_BREP_FLATTEN_PERIPH_OPTION;  
        options.opt_data[0] = tol;  
        while ((object = select_a_face("Select face")) != NULL_TAG)  
        {  
...
> 这段代码是NX的二次开发示例，主要功能包括：
>
> 1. 函数封装：定义了ECHO、report_error等辅助函数，用于打印信息和错误处理。
> 2. 选择面：定义了mask_for_faces和select_a_face函数，用于通过对话框选择一个面。
> 3. 输出函数：定义了WRITE系列函数，用于在列表窗口输出各种类型的数据。
> 4. BREP信息报告：定义了report_brep_topology、report_brep_states、report_brep_geometry等函数，用于递归遍历BREP结构，并输出其拓扑和几何信息。
> 5. 主函数：do_it函数是主函数，循环选择面，并调用UF_BREP_ask_topology查询BREP信息，然后调用report_brep_topology_tree_geometry递归输出BREP信息。
> 6. 初始化与清理：ufusr是NX的入口函数，初始化后调用do_it，最后清理。
> 7. 卸载函数：ufusr_ask_unload用于在卸载时立即卸载。
>
> 总体来看，这段代码通过NX的BREP接口，实现了查询和输出BREP信息的功能，并且包含了错误处理、对话框交互等完整的二次开发流程。
>
