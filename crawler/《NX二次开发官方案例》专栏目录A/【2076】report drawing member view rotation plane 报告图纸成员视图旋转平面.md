### 【2076】report drawing member view rotation plane 报告图纸成员视图旋转平面
...
```cpp
    ...
    {  
        ...
        double  
            ...
            abs_y_vector[3],  
            abs_z_vector[3];  
        while ((view = select_a_view("Report DMV Rotation Plane")) != NULL_TAG)  
        {  
            UF_CALL(UF_DRAW_ask_dmv_rotation_plane(view, &plane));  
            RX(plane);  
            if (plane != NULL_TAG)  
            {  
                UF_CALL(UF_SO_ask_x_direction_of_xform(plane, abs_x_vector));  
                WRITE3F(abs_x_vector);  
                UF_CALL(UF_SO_ask_y_direction_of_xform(plane, abs_y_vector));  
                WRITE3F(abs_y_vector);  
                UF_CALL(UF_SO_ask_z_direction_of_xform(plane, abs_z_vector));  
                WRITE3F(abs_z_vector);  
            }  
        }  
    }  
    /* qq3123197280 */  
...
>
> 1. 定义了一些宏和函数，用于报告错误、输出变量等。
> 2. 定义了一个函数select_a_view()，用于提示用户选择一个视图。
> 3. 定义了一个函数report_object_info()，用于报告一个对象的信息，包括名称、类型、所属部件等。
> 4. 主函数do_it()会循环提示用户选择视图，对于每个选择的视图，会报告视图的DMV旋转平面信息，包括平面的绝对坐标方向。
> 5. ufusr()是NX调用的主函数，初始化NX环境后调用do_it()，然后终止NX环境。
> 6. ufusr_ask_unload()函数用于在卸载用户库时立即卸载。
>
> 总体来说，这段代码通过用户交互选择视图，并报告视图的DMV旋转平面信息，实现了对NX视图的二次开发功能。
>
