### 【2474】report selected objects brep topology trees geometry 报告选定物体的BREP拓扑树几何信息
...
static void report_brep_geometry(UF_BREP_geometry_t *geometry)
{
    ...
    switch (geometry->type)
    {
        ...
        case UF_BREP_SPLINE_GEOM:
            ...
            WRITEnF(geometry->geom.spline.num_poles + geometry->geom.spline.order,
                (double *)geometry->geom.spline.knots);
            WRITEnF4(geometry->geom.spline.num_poles,(double *)geometry->geom.spline.poles);
        ...
        case UF_BREP_BSURFACE_GEOM:
            ...
            WRITEnF(geometry->geom.bsurface.num_poles_u + geometry->geom.bsurface.order_u,
                geometry->geom.bsurface.knots_u);
            WRITEnF(geometry->geom.bsurface.num_poles_v + geometry->geom.bsurface.order_v,
                geometry->geom.bsurface.knots_v);
            WRITEnF4(geometry->geom.bsurface.num_poles_u *
                geometry->geom.bsurface.num_poles_v,
                (double *)geometry->geom.bsurface.poles);
            break;
        case UF_BREP_OFFSET_GEOM:
            WRITE_D(UF_BREP_OFFSET_GEOM);
            WRITE_F(geometry->geom.offset.distance);
            ECHO("Offset from (original):\n");
        ...
        case UF_BREP_FORSURF_GEOM:
            ...
            WRITEnF(geometry->geom.foreign.num_doubles,
                geometry->geom.foreign.double_array);
...
static void report_brep_topology_tree_geometry(UF_BREP_topology_t *topology)
{
    ...
    UF_BREP_state_t
        *states;
    UF_BREP_geom_type_t
        source[] = { UF_BREP_OFFSET_GEOM };
    UF_BREP_geom_type_t
        extracted[] = { UF_BREP_BSURFACE_GEOM };
    UF_BREP_mapping_t
        mapping = { 1, source, extracted };
    UF_UI_open_listing_window();
    ...
    if (topology->type > UF_BREP_SHELL_TOPO)
    {
            ...
            &geom_sense, &nstates, &states)))
        {
            ...
            if (geometry.type == UF_BREP_OFFSET_GEOM)
            {
                ECHO("With Offset applied:\n");
                UF_CALL(UF_BREP_free_geometry_data(&geometry, NULL));
                if (!UF_CALL(UF_BREP_ask_geometry(topology, NULL, &mapping,
                    &geometry, &geom_sense, &nstates, &states)))
                {
                    report_brep_states(nstates, states);
                    report_brep_geometry(&geometry);
                }
...
> 这段NX二次开发代码主要实现了以下功能：
>
> 1. 错误报告：定义了错误报告函数，用于输出错误信息。
> 2. 几何选择：提供了用于选择实体和面的函数。
> 3. BREP拓扑结构查询：实现了查询BREP拓扑结构的函数，并递归输出BREP拓扑树的几何信息。
> 4. 几何信息输出：定义了用于输出BREP几何信息的函数，涵盖点、线、面等几何类型。
> 5. 状态信息输出：提供了用于输出BREP状态信息的函数。
> 6. BREP拓扑树递归遍历：实现了递归遍历BREP拓扑树并输出几何信息的函数。
> 7. 主函数：实现了选择实体，查询BREP拓扑结构并输出几何信息的函数。
> 8. NX初始化和终止：实现了NX初始化和终止函数。
> 9. 卸载函数：提供了卸载函数。
>
> 总的来说，该代码通过NX提供的API，实现了查询BREP实体并输出其拓扑和几何信息的功能。
>
