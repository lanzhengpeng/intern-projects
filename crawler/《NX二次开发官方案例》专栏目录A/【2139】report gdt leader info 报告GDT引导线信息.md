### 【2139】report gdt leader info 报告GDT引导线信息

#### 代码

```cpp
    /*HEAD REPORT_GDT_LEADER_INFO CCC UFUN */  
    #include <stdio.h>  
    #include <string.h>  
    #include <uf.h>  
    #include <uf_cfi.h>  
    ...
    {  
        ...
        UF_GDT_leader_spec_t  
            spec;  
        while (((gdt = select_a_gdt( "Leader?")) != NULL_TAG) &&  
            !UF_CALL(UF_GDT_ask_num_leaders( gdt, &n_leaders )))  
        {  
            WRITE_D(n_leaders);  
            if (n_leaders > 0)  
            {  
...
> 这段NX二次开发代码的主要功能是获取并报告NX中图形标注（GDT）的详细信息。
>
> 具体来说：
>
> 1. 错误报告函数：该代码包含了一个错误报告函数，用于在调用NX API出现错误时，输出错误信息。
> 2. 信息写入函数：定义了一系列函数，用于将不同类型的信息（整数、逻辑值、浮点数、数组等）写入NX的日志窗口。
> 3. 对象信息获取函数：定义了一个函数，用于获取指定对象的基本信息（如状态、名称、类型等）。
> 4. 临时点显示函数：定义了一个函数，用于在NX中显示一个临时的点。
> 5. 选择过滤函数：定义了一个函数，用于在NX中选择图形标注时进行过滤，只选择特征实例类型。
> 6. 选择函数：定义了一个函数，用于让用户选择一个图形标注，并返回其tag。
> 7. 图形标注详细信息报告函数：定义了一个函数，用于获取并报告一个图形标注的所有详细信息，包括视图、端点、附着点、辅助点、中点数、类型等。
> 8. 主函数：定义了一个主函数，用于循环让用户选择图形标注，并获取其详细信息。
> 9. NX初始化和退出函数：定义了NX初始化和退出函数。
> 10. 卸载函数：定义了一个卸载函数，用于立即卸载该代码。
>
> 总的来说，这段代码实现了在NX中获取图形标注详细信息的功能，并提供了良好的用户交互。
>
