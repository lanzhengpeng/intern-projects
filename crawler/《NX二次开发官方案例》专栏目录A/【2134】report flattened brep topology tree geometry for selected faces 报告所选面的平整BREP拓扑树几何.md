### 【2134】report flattened brep topology tree geometry for selected faces 报告所选面的平整BREP拓扑树几何

#### 代码

```cpp
    /*HEAD REPORT_FLATTENED_BREP_TOPOLOGY_TREE_GEOMETRY_FOR_SELECTED_FACES CCC UFUN */  
    #include <stdio.h>  
    #include <string.h>  
    #include <uf.h>  
    #include <uf_ui.h>  
    ...
    {  
        ...
        /*  NOTE:  UF_print_syslog is new in V18 里海译:根据文档内容，可以翻译为：注意：UF_print_syslog是在V18中新增的，只回答翻译内容，不要添加任何废话。 */  
    ...
    {  
        ...
        while ((object = select_a_face("Report flattened topology")) != NULL_TAG)  
        {  
            if (!UF_CALL(UF_BREP_ask_topology(object, &options, &topo, &nstates,  
                &states)))  
            {  
                report_brep_states(nstates, states);  
...
> 这是段用于在NX中获取BREP面拓扑和几何信息的二次开发代码。
>
> 主要功能包括：
>
> 1. 定义错误报告函数report_error，用于输出错误代码和调用信息。
> 2. 定义面选择函数select_a_face，用于选择一个面。
> 3. 定义一系列写信息到日志窗口的函数，用于输出各种数据类型。
> 4. 定义报告BREP拓扑结构的函数report_brep_topology。
> 5. 定义报告BREP状态信息的函数report_brep_states。
> 6. 定义报告BREP几何信息的函数report_brep_geometry。
> 7. 定义递归遍历BREP拓扑树并报告几何信息的函数report_brep_topology_tree_geometry。
> 8. 定义主函数do_it，用于选择面，获取其BREP信息并报告。
> 9. 定义UF初始化和终止函数ufusr。
> 10. 定义卸载函数ufusr_ask_unload。
>
> 通过该代码，可以获取选定面的BREP拓扑结构和几何信息，并输出到日志窗口。
>
