### 【2238】report object info by eid 报告EID的对象信息
...
```cpp
    ...
    {  
        ...
        strcpy(descriptor, "Unknown type");  /* In case its not found 里海译:万一找不到，请翻译为"In case its not found"。 */  
        while (!feof(in_file))  
        {  
    ...
    {  
        tag_t  
            object = NULL_TAG;  
        while (prompt_for_an_integer("Report Info?", "EID", (int *)&object))  
            RX(object);  
    }  
    /* qq3123197280 */  
...
> 这段代码是NX二次开发代码，主要用于查询和报告NX对象的信息。具体功能如下：
>
> 1. 初始化和终止：在代码的开始和结束处，分别调用了UF_initialize()和UF_terminate()，用于初始化和终止NX环境。
> 2. 错误报告函数：定义了ECHO和report_error两个函数，用于在窗口和日志中输出信息，以及在出错时报告错误。
> 3. 查询对象信息：定义了RX和RXn宏，以及report_object_info函数，用于查询和报告NX对象的各种信息，如状态、类型、子类型、名称、拥有者、显示属性等。
> 4. 查询特征信息：定义了report_feature_name_type_and_sysname函数，用于查询NX特征的特征名、系统名和类型。
> 5. 查询部件简称：定义了ask_part_shortname函数，用于查询NX部件的简称。
> 6. 查询对象类型描述符：定义了ask_object_type_descriptor函数，用于查询NX对象类型的描述符。
> 7. 整数输入提示：定义了prompt_for_an_integer函数，用于提示用户输入一个整数。
> 8. 主要功能函数：定义了do_it函数，用于循环提示用户输入对象ID，并报告该对象的信息。
> 9. 主函数：定义了ufusr函数，作为二次开发程序的入口，调用do_it函数执行主要功能。
> 10. 卸载函数：定义了ufusr_ask_unload函数，在卸载二次开发程序时调用。
>
> 综上所述，该代码主要用于查询和报告NX对象的各种信息，以便开发者了解对象的详细情况。
>
