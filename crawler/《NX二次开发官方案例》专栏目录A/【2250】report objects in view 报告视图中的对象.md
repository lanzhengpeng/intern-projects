### 【2250】report objects in view 报告视图中的对象
...
```cpp
    ...
    {  
        ...
        switch (type)  
        {  
            case UF_circle_type: /* non-std format in the header file 里海译:non-std format in the header file */  
    ...
    {  
        ...
        for (ii = UF_VIEW_VISIBLE_OBJECTS; ii <= UF_VIEW_MODIFIED_OBJECTS; ii++)  
        {  
            ECHO("Objects of UF_VIEW_cycle_object_t = %d:\n", ii);  
            while (!UF_VIEW_cycle_objects(view, ii, &object) &&  
...
>
> 1. 定义了一些宏，用于报告错误和输出信息到日志窗口。
> 2. 定义了一个函数ask_part_shortname，用于获取部件的短名称。
> 3. 定义了一个函数ask_object_type_descriptor，用于获取NX对象的类型描述符。
> 4. 定义了一个函数report_feature_name_type_and_sysname，用于报告特征的名字、类型和系统名。
> 5. 定义了一个函数report_object_info，用于报告NX对象的信息，包括状态、类型、子类型、拥有部件、可见性等。
> 6. 定义了一个函数do_it，用于遍历当前工作视图的所有可见对象和已修改对象，并报告它们的信息。
> 7. 定义了ufusr函数作为主函数，初始化NX环境，调用do_it函数，然后终止NX环境。
> 8. 定义了ufusr_ask_unload函数，返回立即卸载标志。
>
> 整体来看，这段代码主要用于遍历和报告当前工作视图中的对象信息，展示了NX二次开发的一些基础接口调用。
>
