### 【2478】report selected objects info 报告选择对象信息
...
```cpp
    ...
    {  
        ...
        switch (type)  
        {  
            case UF_circle_type: /* non-std format in the header file 里海译:头文件中的非标准格式翻译如下：
...
> 这段NX二次开发代码主要用于选择和报告NX对象的信息。其主要功能包括：
>
> 1. 选择对象：使用UF_UI库提供的函数，可以弹出对话框让用户选择一个对象，可以选择任何类型的面、边等特征。
> 2. 报告对象信息：选择的对象会被高亮显示，并且代码会报告对象的类型、子类型、名称、拥有部件、可见性、图层、颜色等信息。
> 3. 处理特征对象：若选择的是特征对象，还会报告特征的名字、系统名和类型。
> 4. 错误处理：代码中定义了一个UF_CALL宏，用于检查函数调用是否成功，如果失败则打印错误信息。
> 5. 程序入口：ufusr是程序的入口函数，初始化NX系统后执行主函数do_it，然后终止NX系统。
> 6. 卸载函数：ufusr_ask_unload函数用于定义程序的卸载方式。
> 7. 辅助函数：代码中定义了一些辅助函数，如ECHO用于打印信息，ask_part_shortname用于获取部件的简称等。
>
> 总体来说，这段代码实现了选择任意NX对象并打印其详细信息的功能，具有一定的参考价值。
>
