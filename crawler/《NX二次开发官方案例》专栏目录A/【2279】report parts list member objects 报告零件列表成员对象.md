### 【2279】report parts list member objects 报告零件列表成员对象
...
> 这段代码是一个NX Open的二次开发示例，其主要功能是遍历当前部件中的对象，并输出每个对象的信息。以下是代码的主要逻辑：
>
> 1. 定义了ECHO宏，用于打印信息到日志窗口和系统日志。
> 2. 定义了UF_CALL宏，用于调用NX Open函数，并捕获错误。
> 3. 定义了RX和WRITEnT宏，用于输出对象信息。
> 4. ask_part_shortname函数用于获取部件的简称。
> 5. ask_object_type_descriptor函数用于获取对象类型描述。
> 6. report_feature_name_type_and_sysname函数用于输出特征的名字、类型和系统名。
> 7. report_object_info函数用于输出对象的各种信息，包括状态、类型、所属部件、显示属性等。
> 8. do_it函数用于遍历当前部件中的对象，并调用report_object_info函数输出信息。
> 9. ufusr函数是二次开发的入口函数，调用do_it函数。
> 10. ufusr_ask_unload函数用于卸载二次开发程序。
>
> 通过调用ufusr函数，可以遍历当前部件中的所有对象，并输出每个对象的状态、类型、名称等详细信息，以便进行调试和查看。
>
