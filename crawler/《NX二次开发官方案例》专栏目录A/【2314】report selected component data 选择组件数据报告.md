### 【2314】report selected component data 选择组件数据报告

#### 代码

```cpp
    /*HEAD REPORT_SELECTED_COMPONENT_DATA CCC UFUN */  
    #include <stdio.h>  
    #include <string.h>  
    #include <uf.h>  
    #include <uf_ui.h>  
...
> 这段代码是NX Open的C语言二次开发示例代码，主要功能是选择组件并报告组件的数据信息。
>
> 主要步骤包括：
>
> 1. 包含必要的NX Open头文件。
> 2. 定义错误报告函数report_error，用于打印UF函数的错误信息。
> 3. 定义组件选择时的掩码函数mask_for_components，只允许选择组件。
> 4. 定义组件选择函数select_a_component，使用对话框让用户选择一个组件。
> 5. 定义各种写入信息到列表窗口的宏和函数，如WRITE_S、WRITE3F等。
> 6. 定义报告组件数据函数report_component_data，获取并打印组件的名称、参考集名称、实例名称、原点、坐标系矩阵、变换矩阵等信息。
> 7. 定义主函数do_it，循环选择组件并报告数据。
> 8. 定义ufusr入口函数，初始化后调用主函数，然后终止。
> 9. 定义卸载函数ufusr_ask_unload，直接卸载。
>
> 代码主要演示了NX组件选择、获取组件属性信息以及打印信息到列表窗口等二次开发技巧。
>
