### 【2251】report objects on selectable layers 报告可选图层上的对象
...
```cpp
    ...
    {  
        tag_t  
            object = NULL_TAG;  
        ECHO("Objects on selectable layers:\n");  
...
> 这段代码是NX的二次开发代码，其主要功能是遍历NX中的可选中层上的所有对象，并输出每个对象的信息。具体步骤如下：
>
> 1. 包含了NX二次开发所需的头文件。
> 2. 定义了错误报告函数report_error()，用于在函数执行失败时输出错误信息。
> 3. 定义了报告对象信息的宏RX()，用于输出对象的详细信息。
> 4. 定义了获取对象类型描述的函数ask_object_type_descriptor()，用于获取对象类型的字符串描述。
> 5. 定义了报告特征名称、类型和系统名称的函数report_feature_name_type_and_sysname()。
> 6. 定义了报告对象信息的函数report_object_info()，用于输出对象的各种属性信息。
> 7. 定义了主函数do_it()，用于遍历可选中层上的所有对象并报告信息。
> 8. 定义了ufusr()函数，作为二次开发的入口函数，调用do_it()执行主体功能。
> 9. 定义了卸载函数ufusr_ask_unload()，用于在卸载时立即释放资源。
>
> 总体来说，这段代码通过遍历可选中层上的对象，并报告每个对象的信息，实现了对NX模型中对象信息的查询功能。
>
